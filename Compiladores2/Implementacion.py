...
def generar_arbol_png(root, filename):
    dot = Digraph(comment='Arbol de Derivacion')

    def agregar_nodos(nodo):
        if nodo is not None:
            # Incluir el lexema ademas del simbolo en el nodo
            nodo_label = f"{nodo.symbol} ({nodo.lexeme})" if nodo.lexeme is not None else nodo.symbol
    ...
    print(f"Arbol de derivacion generado y guardado como '{filename}'.")

# Ejemplo de uso
generar_arbol_png(root, 'Arbol.png')
